<template>
	<div class="flex min-w-0 items-center">
		<template v-if="dropdownItems.length">
			...
			<span
				class="ml-1 mr-0.5 text-lg font-bold text-ink-gray-4"
				aria-hidden="true"
			>
				>
			</span>
		...
		>
			<template v-for="(item, i) in crumbs" :key="item.label">
				<router-link
					...
					class="flex items-center rounded px-0.5 py-1 text-lg font-bold focus:outline-none focus-visible:ring-2 focus-visible:ring-outline-gray-3"
					:class="[
						i == crumbs.length - 1
							? 'text-ink-gray-9'
				...
				<span
					...
					class="mx-0.5 text-lg font-bold text-ink-gray-4"
					aria-hidden="true"
				>
					>
				</span>
